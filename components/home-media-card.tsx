...
export const HomeMediaCard = ( { item, onDelete, onClick }: HomeMediaCardProps ) => {
  return (
    <div
      onClick={ onClick }
      className={ cn(
        "shrink-0 snap-start outline-none group relative h-32 w-[85vw] sm:w-[320px] lg:w-85 xl:w-90 bg-card shadow-md p-3 overflow-hidden cursor-pointer transition-all rounded-md",
    ...
    >
      ...
      <div
        ...
        <div className="flex justify-between items-end text-xs text-muted-foreground mt-2">
          <span
            className="bg-muted/80 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] border border-border/50 capitalize whitespace-nowrap">
            { item.status.replace( "_", " " ) }
          </span>
          <span className="text-[10px] shrink-0">{ item.rating }/10</span>
        </div>
      </div>
    </div>
  )
